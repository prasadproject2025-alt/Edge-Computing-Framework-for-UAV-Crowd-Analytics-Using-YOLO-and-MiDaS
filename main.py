...
class DepthEstimatorThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            time.sleep(0.05) # Prevent CPU hogging on the edge device

def calculate_spatial_density(pedestrians, depth_map):
    """
    Calculates true spatial density using 3D Euclidean distance.
    ...
    for (x, y, w, h) in pedestrians:
        ...
        # Extract relative depth at the center coordinate
        z = depth_map[cy, cx] 
    ...
    for i in range(len(points_3d)):
        ...
        for j in range(len(points_3d)):
            ...
            # 3D Euclidean distance (Z is weighted to account for perspective)
...
def main():
    ...
    # Initialize Video Stream (0 for default camera, or path to drone video)
    ...
    while cap.isOpened():
        ...
        # Update shared frame for the background depth thread
        ...
        # Highlight crowded individuals in red
        for (x, y, w, h) in crowded_boxes:
            cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0, 255), 3)
            
        cv2.putText(frame, f"Headcount: {len(pedestrians)}", (20, 40), 
        ...
        # Optional: Picture-in-Picture Depth Map for debugging
